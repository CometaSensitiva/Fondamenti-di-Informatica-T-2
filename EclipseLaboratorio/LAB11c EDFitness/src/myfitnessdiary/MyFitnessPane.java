...
public class MyFitnessPane extends BorderPane {
	...
	public MyFitnessPane(Controller controller) {
		...
		activitiesComboBox = new ComboBox<>(FXCollections.observableArrayList(controller.getActivities()));
		activitiesComboBox.setEditable(false);
		activitiesComboBox.setValue(controller.getActivities().iterator().next());

		intensitiesComboBox = new ComboBox<>(FXCollections.observableArrayList(Intensity.values()));
		...
		durationSpinner = new Spinner<>(0, 1000, 0, 5);
	...
	private void myHandle(ActionEvent event) {
		...
		if (date == null) {
			controller.alert("Dati errati", "Data errata", "La data inserita non � corretta");
			return;
		}
		Integer duration = durationSpinner.getValue();
		if (duration <= 0) {
			controller.alert("Dati errati", "Durata non valida", "Una durata nulla o negativa non � ammissibile");
			return;
		}
		Intensity intensity = intensitiesComboBox.getValue();
		if (intensity == null) {
			controller.alert("Dati errati", "Intensit� nulla", "Una intensit� negativa non � ammissibile");
			return;
		}
		Activity activity = controller.getActivity(activitiesComboBox.getValue());
		if (activity == null) {
			controller.alert("Dati errati", "Attivit� inesistente", "Non esiste alcuna attivit� con questo nome");
			return;
		}
